...
MAIN_ROUTES = [
    ["白音华东站", "哈日努拉站", "霍林郭勒站", "白音胡硕站", "大林站",
     "五道木站", "通辽站", "花吐古拉站", "双泡子站", "金宝图站",
     "水泉站", "遂辽站", "甘旗卡站", "彰武站", "新立电站",
     "黑山站", "大虎山站", "芳山堡站", "锦州港入口", "秦皇岛港"],
...
def generate_train_positions(num_trains, num_frames, hour_start, hour_end):
    ...
    for i in range(num_trains):
        ...
        start_progress = np.random.uniform(0, 0.8)
        speed = np.random.uniform(0.008, 0.015)
        is_loaded = np.random.random() < 0.6
        trains.append({
            "route": route,
    ...
    for frame in range(num_frames):
        frame_trains = []
        for t in trains:
            progress = (t["start_progress"] + frame * t["speed"]) % 1.0
...
def create_simple_network_animation(metrics_llm, closure_start=48, closure_end=120):
    ...
    for frame_idx in range(num_frames):
        hour = frame_idx * 4
        inflow = metrics_llm.hourly_inflow[min(hour, len(metrics_llm.hourly_inflow)-1)]
        # 入港流量越高，重车比例越高（正常0.92万吨/h → 60%重车）
        loaded_ratio = min(0.85, max(0.3, inflow / (22.0/24) * 0.6))
        for t in train_frames[frame_idx]:
            t["loaded"] = np.random.random() < loaded_ratio
    ...
    loaded_x = [t["x"] for t in initial if t["loaded"]]
    loaded_y = [t["y"] for t in initial if t["loaded"]]
    empty_x = [t["x"] for t in initial if not t["loaded"]]
    empty_y = [t["y"] for t in initial if not t["loaded"]]

    loaded_trace_idx = len(fig.data)
    fig.add_trace(go.Scatter(
        x=loaded_x, y=loaded_y,
        mode="markers", name="重车(载煤运行)",
        marker=dict(size=10, color="#c0392b", symbol="circle",
                    line=dict(width=1.5, color="#922b21")),
    ))

    empty_trace_idx = len(fig.data)
    fig.add_trace(go.Scatter(
        x=empty_x, y=empty_y,
        mode="markers", name="空车(返回装车)",
        marker=dict(size=8, color="#27ae60", symbol="circle-open",
                    line=dict(width=2, color="#1e8449")),
    ))
    ...
    for frame_idx in range(num_frames):
        ...
        lx = [t["x"] for t in ft if t["loaded"]]
        ly = [t["y"] for t in ft if t["loaded"]]
        ex = [t["x"] for t in ft if not t["loaded"]]
        ey = [t["y"] for t in ft if not t["loaded"]]

        frames.append(go.Frame(
            data=[
                go.Scatter(x=lx, y=ly, mode="markers",
                           marker=dict(size=10, color="#c0392b", symbol="circle",
                                       line=dict(width=1.5, color="#922b21"))),
                go.Scatter(x=ex, y=ey, mode="markers",
                           marker=dict(size=8, color="#27ae60", symbol="circle-open",
                                       line=dict(width=2, color="#1e8449"))),
            ],
            traces=[loaded_trace_idx, empty_trace_idx],
            name=f"h{hour}",
        ))
    ...
    fig.update_layout(
        ...
        sliders=[dict(
            active=0,
            steps=[dict(
                args=[[f"h{i*4}"], {"frame": {"duration": 250, "redraw": True},
                                     "mode": "immediate"}],
                label=f"{i*4}h" if i % 3 == 0 else "",
                method="animate")
                for i in range(num_frames)],
